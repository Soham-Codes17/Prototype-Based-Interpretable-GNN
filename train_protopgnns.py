...
class TrainModel(object):
    ...
    # ----------------------------------------------------------
    # EMBEDDING LOSS (WITH NEW DIVERSITY LOSS ADDED)
    # ----------------------------------------------------------
    def __emb_loss__(self, data, out, mask=None):
        ...
        if self.graph_classification:
            ...
            for g in range(n_graphs):
                ...
                L_compactness += torch.min(
                    l2s[nodes, label*self.model.num_basis_per_class:
                         (label+1)*self.model.num_basis_per_class]
                )
                ...
                if label != self.model.output_dim - 1:
                    L_separation += -torch.min(
                        l2s[nodes, (label+1)*self.model.num_basis_per_class:]
                    )

            L_compactness /= n_graphs
            L_separation /= n_graphs
            ...
            for node in range(n_nodes):
                ...
                L_compactness += torch.min(
                    l2s[node, label*self.model.num_basis_per_class:
                           (label+1)*self.model.num_basis_per_class]
                )
                ...
                if label != self.model.output_dim - 1:
                    L_separation += -torch.min(
                        l2s[node, (label+1)*self.model.num_basis_per_class:]
                    )

            L_compactness /= n_nodes
            L_separation /= n_nodes
            L_cs = (0.8 * L_compactness + 0.08 * L_separation)

        # ------------------------------------
        # ADD NEW PROTOTYPE DIVERSITY LOSS
        # ------------------------------------
        div_weight = 0.05       # You can tune this (0.01–0.1)
        L_div = self.__diversity_loss__()

        L_total = L_id + L_cs + div_weight * L_div
        return L_total

    # ----------------------------------------------------------
    # DIVERSITY LOSS FUNCTION (NEW)
    # ----------------------------------------------------------
    def __diversity_loss__(self):
        """
        Prototype Diversity Loss:
        Encourages prototypes to be different by penalizing cosine similarity.
        """
        if not hasattr(self.model, 'basis_concepts') or self.model.basis_concepts is None:
            return torch.tensor(0.0, device=self.device)

        P = self.model.basis_concepts  # shape [embedding_dim, num_prototypes]

        if P.numel() == 0:
            return torch.tensor(0.0, device=self.device)

        P_norm = F.normalize(P, dim=0)       # normalize column-wise
        sim = torch.matmul(P_norm.T, P_norm)  # cosine similarity matrix
        diag = torch.eye(sim.size(0), device=sim.device)

        off_diag = sim - diag                # remove diagonal
        positive_sim = off_diag[off_diag > 0]

        if positive_sim.numel() == 0:
            return torch.tensor(0.0, device=sim.device)

        return positive_sim.sum()

    # ----------------------------------------------------------
    # CLASSIFIER LOSS (UNCHANGED)
    # ----------------------------------------------------------
    ...
    def train(self, train_params=None, optimizer1_params=None, optimizer2_params=None):
        ...
        for epoch in range(num_epochs):
            ...
            for t in range(N1):
                ...
                with torch.no_grad():
                    self.model.basis_concepts.div_(
                        torch.linalg.norm(self.model.basis_concepts, dim=0)
                    )
...
def main(config):
    ...
    optimizer1_params = {
        'lr': config.models.param.learning_rate1,
        'weight_decay': config.models.param.weight_decay1
    }

    optimizer2_params = {
        'lr': config.models.param.learning_rate2,
    ...
    if config.models.param.graph_classification:
        trainer = TrainModel(
            ...
            save_dir=os.path.join(config.models.gnn_saving_dir, save_dir_name),
